...
const leftStateMap: { [key: string]: vscode.TextEditorLineNumbersStyle } = {
  absRel: vscode.TextEditorLineNumbersStyle.On,
  relAbs: vscode.TextEditorLineNumbersStyle.Relative,
...
const rightStateMap: { [key: string]: vscode.TextEditorLineNumbersStyle } = {
  absRel: vscode.TextEditorLineNumbersStyle.Relative,
  relAbs: vscode.TextEditorLineNumbersStyle.On,
...
class LineNumberManager {
  ...
  */
  decorTypeMap: Map<vscode.TextEditor, Map<number, vscode.TextEditorDecorationType>>
   ...
   */
  getRightState() {
    return vscode.workspace.getConfiguration().get('editor.lineNumbers', rightStateMap.off)
  }
  ...
  setRightState(key: string) {
    ...
    vscode.workspace.getConfiguration().update('editor.lineNumbers', val, vscode.ConfigurationTarget.Global)
  }
   ...
   */
  getLeftState() {
    return this.context.globalState.get(this.LEFT_STATE_STORE, leftStateMap.off)
  }
  ...
  updateDecor(editor: vscode.TextEditor) {
    ...
    const end = Math.min(editor.visibleRanges[0].end.line + 1, editor.document.lineCount - 1)
    ...
    for (let i = start; i <= end; ++i) {
      ...
      if (this.getLeftState() === vscode.TextEditorLineNumbersStyle.Off) {
        ...
        num = i + 1
      } else if (this.getLeftState() === vscode.TextEditorLineNumbersStyle.Relative) {
    ...
    for (let i = start; i <= end; ++i) {
      if (this.decorNumMap.get(editor)!.has(i)) {
        editor.setDecorations(this.decorTypeMap.get(editor)!.get(i)!, [new vscode.Range(i, 0, i, 0)])
      }
    }
  }
...
function registerCommands(mgr: LineNumberManager) {
  for (let key in names) {
    ...
    let disposable = vscode.commands.registerCommand(`vscode-double-line-numbers.${key}`, () => {
      vscode.window.showInformationMessage(`Setting line numbers to ${name}!`)
...
function getSvgUri(num: number) {
  ...
  const fill = encodeURIComponent(
    vscode.workspace.getConfiguration().get('vscode-double-line-numbers.font.color', '#858585'),
  )
  const fontWeight = vscode.workspace.getConfiguration().get('vscode-double-line-numbers.font.weight')
  const fontFamily = vscode.workspace
    .getConfiguration()
    .get('vscode-double-line-numbers.font.family', vscode.workspace.getConfiguration().get('editor.fontFamily'))
  const textLength = vscode.workspace.getConfiguration().get('vscode-double-line-numbers.text.width')
  const fontSize = vscode.workspace.getConfiguration().get('vscode-double-line-numbers.text.height')
